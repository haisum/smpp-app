...
import (
	...
	"bitbucket.org/codefreak/hsmpp/pkg/db/models/settings"
	"bitbucket.org/codefreak/hsmpp/pkg/stringutils"
)

// Config represents settings for pkg connections
type Config struct {
...
// Conn represents configuration specific to a single pkg connection
type Conn struct {
	ID       string
	URL      string
	User     string
...
// PduFields are fields that may be sent to pkg server
// when sending an sms. These are usually optional but some pkg providers
// require them.
